 ...
 * the static Kitchen Favorites catalog, contact info, and wires up
 * the daily menu + festival settings modules.
 * -----------------------------------------------------------------------
 */
...
function applySiteCopy() {
  ...
  if (metaDesc) {
    metaDesc.setAttribute("content", SITE.metaDescription);
  }
  ...
  document.querySelectorAll("[data-contact-phone]").forEach((el) => {
    ...
    if (el.tagName === "A") {
      el.href = `tel:${SITE.contact.phone.replace(/\s+/g, "")}`;
    }
  });
...
function catalogCard(item) {
  const fallbackImage = item.image.replace(
    /\.jpg$/i,
    ".svg"
  );
  ...
  try {
    ...
    container.innerHTML = `
      <div class="menu-empty">
        ...
        <p class="menu-empty__sub">
          Please refresh, or call us and we'll tell you what's cooking.
        </p>
      </div>
    `;
  }
...
document.addEventListener(
  ...
  () => {
    ...
    // New simple Festival Management system
    initFestivalBanner();
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    navigator.serviceWorker
      ...
      .then(() => {
        console.log(
          "Charroti service worker registered."
        );
      })
      .catch((error) => {
        console.error(
...
window.addEventListener(
  ...
  () => {
    console.log(
      "Charroti installed successfully."
    );
